...
import { Scale, Plus, Trash2, Upload, Loader2, Pencil, Circle, CircleDashed, CheckCircle2, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function LawReading() {
  ...
  const saveEdit = async () => {
    ...
    setItems(prev =>
      prev.map(i => (i.id === editItem.id ? { ...i, law, articles } : i)),
    );
    setEditItem(null);
    const { error } = await supabase
      .from('law_readings')
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const parsed: { law: string; articles: string }[] = data?.items ?? [];
                ...
                <li key={item.id} className="flex items-center gap-2 rounded-lg border border-border/60 bg-background px-3 py-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      {item.articles || 'Sem artigos definidos'} · {meta.label}
                    </p>
                  </div>
                  <Button
                    variant="ghost"
